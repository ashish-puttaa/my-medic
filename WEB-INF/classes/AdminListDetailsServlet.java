...
public void doGet(HttpServletRequest request, HttpServletResponse response)  throws IOException, ServletException {
        ...
        try {
                ...
                if(selected.equals("viewpatients")) {
                        ...
                        ArrayList<PatientBean> patient_list;

                        if(search_key != null) {
                                patient_list = ob.getAllPatients(search_key);
                        }
                        else
                                patient_list = ob.getAllPatients();
                        ...
                        request.setAttribute("style", "<style> .table-card { margin: 3em calc(50vw - (1221px/2)); } </style>");
                        ...
                        if(download_flag != null && download_flag.equals("true")) {
                                ...
                                request.setAttribute("file_heading", "Patients");
                                request.setAttribute("file_name", "patients");
                ...
                if(selected.equals("viewdoctors") || selected.equals("removedoctors")) {
                        ...
                        ArrayList<DoctorBean> doctor_list;
                        if(search_key != null) {
                        ...
                        if(download_flag != null && download_flag.equals("true")) {
                                request.setAttribute("redirect_url", "listastable.jsp");
                                request.setAttribute("file_heading", "Doctors");
                                request.setAttribute("file_name", "doctors");
                                RequestDispatcher requestDispatcher = request.getRequestDispatcher("/Download");
                                requestDispatcher.forward(request, response);
                                return;
                        }
                ...
                if(selected.equals("viewappointments")) {
                        ...
                        if(search_key != null) {
                                appointment_list = ob.getAllAppointments(search_key);
                        }
                        else
                                appointment_list = ob.getAllAppointments();
                        ...
                        request.setAttribute("list", list);
                        request.setAttribute("heading", "Appointments");
                        request.setAttribute("style", "<style> .table-card { margin: 3em 24vw; } </style>");

                        // Redirecting to DownlaodServlet if Download option was selected
                        String download_flag = request.getParameter("download_flag");
                        if(download_flag != null && download_flag.equals("true")) {
                                request.setAttribute("redirect_url", "listastable.jsp");
                                request.setAttribute("file_heading", "Appointments");
                                request.setAttribute("file_name", "appointments");
                                RequestDispatcher requestDispatcher = request.getRequestDispatcher("/Download");
                                requestDispatcher.forward(request, response);
                                return;
                        }
