...
public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        try {
                String download_flag = request.getParameter("download_flag");

                String read_flag = request.getParameter("read_flag");

                if(read_flag != null && read_flag.equals("true")) {
                        // System.out.println("\n\n\nREAD DETECTED");
                        NotificationDAO nob = NotificationDAO.getInstance();
                        String user_id = (String) session.getAttribute("user_id");
                        nob.changeNotificationsToRead(user_id);
                        // System.out.println("\n\n\nUSER ID : " + user_id + "\n\n\n\n");
                }

                if(user.equals("doctor")) {
                        Options option = Options.MANAGE;
                        ...
                        if(search_key != null) {
                                appointment_list = dob.getAppointments(DB, search_key);
                        }
                        else
                                appointment_list = dob.getAppointments(DB);
                        ...
                        while(iter.hasNext()) {
                                ...
                                if(selected.equals("doctor_open_app")) {
                                        ...
                                        // Setting download file properties
                                        if(download_flag != null && download_flag.equals("true")) {
                                                request.setAttribute("file_heading", "Open Appointments");
                                                request.setAttribute("file_name", "OpenAppointments");
                                        }
                                }
                                ...
                                if(selected.equals("doctor_all_app")) {
                                        ...
                                        // Setting download file properties
                                        if(download_flag != null && download_flag.equals("true")) {
                                                request.setAttribute("file_heading", "All Appointments");
                                                request.setAttribute("file_name", "AllAppointments");
                                        }
                                }
                                ...
                                if(selected.equals("doctor_manage_app")) {
                                        ...
                                        // Setting download file properties
                                        if(download_flag != null && download_flag.equals("true")) {
                                                request.setAttribute("file_heading", "Pending Appointments");
                                                request.setAttribute("file_name", "PendingAppointments");
                                        }
                                }
                        }
                        ...
                        // Redirecting to DownlaodServlet if Download option was selected
                        if(download_flag != null && download_flag.equals("true")) {
                                RequestDispatcher requestDispatcher = request.getRequestDispatcher("/Download");
                                requestDispatcher.forward(request, response);
                                return;
                        }

                        // Redirecting to SearchSerlvet if Search option was selected
                ...
                if(user.equals("patient")) {
                        ...
                        if(selected.equals("patient_open_app")) {
                                ...
                                ArrayList<AppointmentBean> appointment_list;

                                if(search_key != null) {
                                        appointment_list = pob.getAppointments(PB, search_key);
                                }
                                else
                                        appointment_list = pob.getAppointments(PB);
                                ...
                                // Redirecting to DownlaodServlet if Download option was selected
                                if(download_flag != null && download_flag.equals("true")) {
                                        request.setAttribute("file_heading", "Patient's Open Appointments");
                                        request.setAttribute("file_name", "PatientOpenAppointments");
                                        RequestDispatcher requestDispatcher = request.getRequestDispatcher("/Download");
                                        requestDispatcher.forward(request, response);
                                        return;
                                }
